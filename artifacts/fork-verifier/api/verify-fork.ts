...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    // Fast path: check if the fork exists under the user's account with the same name
    ...
    // Fallback: scan paginated forks list (handles renamed forks)
    let found = false;
    ...
    for (let page = 1; page <= 5 && !found; page++) {
      ...
      const match = forks.find(
        (f) => f.owner.login.toLowerCase() === cleanUsername.toLowerCase()
      );
    ...
    return res
      .status(500)
      .json({ error: "Failed to contact GitHub API. Please try again." });
  }
}
